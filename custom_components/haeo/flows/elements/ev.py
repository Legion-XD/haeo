...
def _build_trip_entity_fields(
    subentry_data: dict[str, Any] | None = None,
) -> dict[str, tuple[vol.Marker, Any]]:
    """Build trip entity selector field entries for the config flow."""
    ...
    return {
        CONF_CALENDAR_ENTITY: (
            vol.Required(CONF_CALENDAR_ENTITY),
            EntitySelector(EntitySelectorConfig(domain="calendar")),
        ),
        CONF_CONNECTED: (
            vol.Required(CONF_CONNECTED),
            EntitySelector(EntitySelectorConfig(domain="binary_sensor")),
        ),
        CONF_ODOMETER: (
            vol.Required(CONF_ODOMETER),
            EntitySelector(EntitySelectorConfig(domain="sensor")),
        ),
        CONF_ODOMETER_AT_DISCONNECT: (
            vol.Required(CONF_ODOMETER_AT_DISCONNECT),
            EntitySelector(EntitySelectorConfig(domain="sensor")),
        ),
    }
...
class EvSubentryFlowHandler(ElementFlowMixin, ConfigSubentryFlow):
    ...
    def _build_schema(
        ...
        top_level = {
            ...
            **_build_trip_entity_fields(subentry_data),
        }
    ...
    def _build_defaults(
        ...
        defaults: dict[str, Any] = {
            ...
            CONF_CALENDAR_ENTITY: trip_data.get(CONF_CALENDAR_ENTITY, ""),
            CONF_CONNECTED: trip_data.get(CONF_CONNECTED, ""),
            CONF_ODOMETER: trip_data.get(CONF_ODOMETER, ""),
            CONF_ODOMETER_AT_DISCONNECT: trip_data.get(CONF_ODOMETER_AT_DISCONNECT, ""),
            **build_sectioned_choose_defaults(
                self._get_sections(),
                input_fields,
                current_data=subentry_data,
    ...
    def _build_config(self, user_input: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            SECTION_TRIP: {
                ...
                CONF_CONNECTED: user_input[CONF_CONNECTED],
                CONF_ODOMETER: user_input[CONF_ODOMETER],
                CONF_ODOMETER_AT_DISCONNECT: user_input[CONF_ODOMETER_AT_DISCONNECT],
            },
            **config_dict,
        }
