...
def _apply_connected_mask(
    ...
    connected_flag: NDArray[np.floating[Any]] | None,
    ...
    return float(value) * connected_flag


def _invert_flag(
    flag: NDArray[np.floating[Any]] | None,
) -> NDArray[np.floating[Any]] | None:
    """Invert a binary flag array (1.0 → 0.0, 0.0 → 1.0)."""
    if flag is None:
        return None
    return 1.0 - flag
